...
import apps.experienceaem.core.ids.IDSService;
import apps.experienceaem.core.workflows.EAEMPostRequestToIDS;
...
@Component(
        ...
        property = {
                "job.topics=" + EAEMPostRequestToIDS.INDESIGN_SERVER_TOPIC,
        }
)
public class IDSJobConsumer implements JobConsumer {
    ...
    public JobResult process(Job job) {
        ...
        try {
            resolver = resolverFactory.getServiceResourceResolver(EAEMPostRequestToIDS.INDESIGN_AUTH_INFO);
    ...
    private JobResult executeCreateInDesignDoc(Job job, ResourceResolver resolver) throws Exception{
        ...
        scriptArs.put(EAEMPostRequestToIDS.CONTENT_JSON, (String)job.getProperty(EAEMPostRequestToIDS.CONTENT_JSON));
